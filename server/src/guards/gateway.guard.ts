...
import { AuthPayload, SocketWithAuth } from "src/polls/types/types";
...
export class GatewayAdminGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        try {
            ...
            this.logger.debug("Validating Voter Token")
            ...
            if (sub !== poll.adminID){
                 throw new WsUnauthorizedException("Admin Authorization Required")
            }
            return true
        } catch (error) {
            throw new WsUnauthorizedException("Admin Authorization Required")
